...
#include "fix_pimreg_cg_lr_cut.h"
...
FixPIMRegCGLRCUT::FixPIMRegCGLRCUT(LAMMPS *lmp, int narg, char **arg) :
    Fix(lmp, narg, arg), id_pe(nullptr), pe(nullptr), rk(nullptr), 
    tempk(nullptr), pk(nullptr), pim(nullptr), rkp1(nullptr), bk(nullptr), tempf(nullptr)
{
...
FixPIMRegCGLRCUT::~FixPIMRegCGLRCUT()
{
...
void FixPIMRegCGLRCUT::init_list(int /*id*/, NeighList *ptr)
{
  list = ptr;
}
...
void FixPIMRegCGLRCUT::init()
{
  ...
  // Get a pointer to the PairPIMLR class to obtain pair-wise coefficient
  pim = dynamic_cast<PairPIMLR *>(force->pair_match("^pim/lr",0));
  if (pim == nullptr)
  error->all(FLERR,"Must use pair_style pim/lr with fix pimreg/cg/lr/cut");
...
void FixPIMRegCGLRCUT::setup(int vflag)
{ 
  pre_force(vflag);
  init_ek_flag = 1;
}
...
void FixPIMRegCGLRCUT::pre_force(int /* vflag */)
{
...
void FixPIMRegCGLRCUT::calculate_dipoles()
{
...
void FixPIMRegCGLRCUT::calculate_b(double **b, double **v, PairPIMLR *pim)
{
...
void FixPIMRegCGLRCUT::calculate_AdotV(double **Adotv, double **v, PairPIMLR *pim)
{
...
---------------------------------------------------------------------- */
double FixPIMRegCGLRCUT::dot_local(double **a, double **b, int maxatom)
{ 
...
double FixPIMRegCGLRCUT::update_energy()
{
...
void FixPIMRegCGLRCUT::grad_clear(double **array)
{
...
void FixPIMRegCGLRCUT::reallocate()
{
  memory->destroy(tempk);
  memory->destroy(rk);
  memory->destroy(pk);
...
double FixPIMRegCGLRCUT::memory_usage()
{
  double bytes = 0.0;
...
int FixPIMRegCGLRCUT::pack_forward_comm(int n, int *list, double *buf,
...
void FixPIMRegCGLRCUT::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  double **mu = atom->mu;
  m = 0;
...
void FixPIMRegCGLRCUT::calculate_AdotV_one(double **Adotv, double **x, double **v, int i, int *jlist, int jnum, PairPIMLR *pim)
{ 
  double tempSum;
  int j;
  double xtmp,ytmp,ztmp;
...
Diagnostic printing 
-----------------------------------------------------------------*/
void FixPIMRegCGLRCUT::printA(PairPIMLR *pim)
{ 
  int i, j, idim, jdim;
  int nmax = atom->nmax;
  int nlocal = atom->nlocal;
...
void FixPIMRegCGLRCUT::calculate_b_one(double **b, double **x, double **mu, int i, int *jlist, int jnum, PairPIMLR *pim)
{
  double tempSum;
  int j;
  double xtmp,ytmp,ztmp;
...
void FixPIMRegCGLRCUT::calculate_diff(double **dedmu, PairPIMLR *pim)
{
  int ii, i, inum, jnum,jj, itype;
  double xtmp,ytmp,ztmp;
  int *ilist,*jlist,*numneigh,**firstneigh;
