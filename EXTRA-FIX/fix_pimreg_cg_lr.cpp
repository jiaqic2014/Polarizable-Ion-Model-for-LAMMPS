...
FixPIMRegCGLR::FixPIMRegCGLR(LAMMPS *lmp, int narg, char **arg) :
...
void FixPIMRegCGLR::init_list(int /*id*/, NeighList *ptr)
{
  list = ptr;
}
...
void FixPIMRegCGLR::init()
{
  ...
  pim = dynamic_cast<PairPIMLR *>(force->pair_match("^pim",0));
  if (pim == nullptr)
  ...
  ewald = dynamic_cast<EwaldCG *>(force->kspace_match("^ewald/cg",0));
  if (ewald == nullptr)
  error->all(FLERR,"Must use kspace_style ewald/cg with fix pimreg/cg/lr");
...
void FixPIMRegCGLR::setup(int vflag)
{ 
  pre_force(vflag);
  init_ek_flag = 1;
}
...
void FixPIMRegCGLR::pre_force(int /* vflag */)
{
...
void FixPIMRegCGLR::calculate_dipoles()
{
...
void FixPIMRegCGLR::calculate_b(double **b, double **v, PairPIMLR *pim)
{
...
void FixPIMRegCGLR::calculate_AdotV(double **Adotv, double **v, PairPIMLR *pim)
{
...
---------------------------------------------------------------------- */
double FixPIMRegCGLR::dot_local(double **a, double **b, int maxatom)
{ 
...
double FixPIMRegCGLR::update_energy()
{
...
void FixPIMRegCGLR::grad_clear(double **array)
{
...
double FixPIMRegCGLR::memory_usage()
{
  double bytes = 0.0;
...
int FixPIMRegCGLR::pack_forward_comm(int n, int *list, double *buf,
...
void FixPIMRegCGLR::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  double **mu = atom->mu;
  m = 0;
...
void FixPIMRegCGLR::calculate_AdotV_one(double **Adotv, double **x, double **v, int i, int *jlist, int jnum, PairPIMLR *pim)
{ 
  double tempSum;
  int j;
  double xtmp,ytmp,ztmp;
...
Diagnostic printing 
-----------------------------------------------------------------*/
void FixPIMRegCGLR::printA(PairPIMLR *pim)
{ 
  int i, j, idim, jdim;
  int nmax = atom->nmax;
  int nlocal = atom->nlocal;
...
void FixPIMRegCGLR::calculate_b_one(double **b, double **x, double **mu, int i, int *jlist, int jnum, PairPIMLR *pim)
{
  double tempSum;
  int j;
  double xtmp,ytmp,ztmp;
...
void FixPIMRegCGLR::calculate_diff(double **dedmu, PairPIMLR *pim)
{
  int ii, i, inum, jnum,jj, itype;
  double xtmp,ytmp,ztmp;
  int *ilist,*jlist,*numneigh,**firstneigh;
