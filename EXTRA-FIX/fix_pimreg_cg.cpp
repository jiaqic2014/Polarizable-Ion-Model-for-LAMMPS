...
/* ----------------------------------------------------------------------
   Contributing author: Jiaqi Chen (University of Shanghai for Science and Technology)
------------------------------------------------------------------------- */

#include "fix_pimreg_cg.h"
#include "pair_pim.h"
...
FixPIMRegCG::FixPIMRegCG(LAMMPS *lmp, int narg, char **arg) :
...
void FixPIMRegCG::init_list(int /*id*/, NeighList *ptr)
{
  list = ptr;
}
...
void FixPIMRegCG::init()
{
  ...
  if (pim == nullptr)
    error->all(FLERR,"Must use pair_style pim with fix pimreg/cg");
...
void FixPIMRegCG::setup(int vflag)
{ 
  pre_force(vflag);
}
...
void FixPIMRegCG::pre_force(int /* vflag */)
{
...
void FixPIMRegCG::calculate_dipoles()
{
  ...
  while(iter<maxiteration){
    ...
    //tempdot = dot_local(rk,rkp1,nlocal);
    //Diagnostic, rk should be perpendicular with rkp1
    //if(comm->me==0){
    //  auto mesg = fmt::format("PIMReg Diagnostic: rk*rkp1 = {:e}\n",tempdot);
    //  utils::logmesg(lmp,mesg);
    //}

    //updating energy under the new configuration. 
    //energy = update_energy();
...
void FixPIMRegCG::calculate_b(double **b, PairPIM *pim)
{
...
/* ----------------------------------------------------------------------
   calculate the temp = A*v at the local node
---------------------------------------------------------------------- */

void FixPIMRegCG::calculate_AdotV(double **Adotv, double **v, PairPIM *pim)
{
...
---------------------------------------------------------------------- */
double FixPIMRegCG::dot_local(double **a, double **b, int maxatom)
{ 
...
double FixPIMRegCG::update_energy()
{
...
void FixPIMRegCG::grad_clear(double **array)
{
...
double FixPIMRegCG::memory_usage()
{
  double bytes = 0.0;
...
int FixPIMRegCG::pack_forward_comm(int n, int *list, double *buf,
...
void FixPIMRegCG::unpack_forward_comm(int n, int first, double *buf)
{
  int i,m,last;
  double **mu = atom->mu;
  m = 0;
...
void FixPIMRegCG::calculate_AdotV_one(double **Adotv, double **x, double **v, int i, int *jlist, int jnum, PairPIM *pim)
{ 
  double tempSum;
  int j;
  double xtmp,ytmp,ztmp;
...
Diagnostic printing 
-----------------------------------------------------------------*/
void FixPIMRegCG::printA(PairPIM *pim)
{ 
  int i, j, idim, jdim;
  int nmax = atom->nmax;
  int nlocal = atom->nlocal;
...
void FixPIMRegCG::calculate_b_one(double **b, double **x, double **mu, int i, int *jlist, int jnum, PairPIM *pim)
{
  double tempSum;
  int j;
  double xtmp,ytmp,ztmp;
...
void FixPIMRegCG::calculate_diff(double **dedmu, PairPIM *pim)
{
  int ii, i, inum, jnum,jj, itype;
  double xtmp,ytmp,ztmp;
  int *ilist,*jlist,*numneigh,**firstneigh;
