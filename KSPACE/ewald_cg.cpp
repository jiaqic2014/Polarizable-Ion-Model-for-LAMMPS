...
/* ----------------------------------------------------------------------
   Contributing authors: Pieter in 't Veld (SNL), Stan Moore (SNL), Jiaqi Chen (jiaqic@usst.edu.cn)
------------------------------------------------------------------------- */
...
EwaldCG::EwaldCG(LAMMPS *lmp) : KSpace(lmp),
  kenergy(nullptr), kvirial(nullptr), energy_self_peratom(nullptr), virial_self_peratom(nullptr),
  ekr_local(nullptr), hvec(nullptr), kvec(nullptr), B(nullptr), cek_local(nullptr), cek_global(nullptr), ek(nullptr)
{
...
void EwaldCG::settings(int narg, char **arg)
{
...
double EwaldCG::rms(int km, double prd, bigint natoms,
...
void EwaldCG::deallocate_peratom()                        // free memory
{
...
/* -----------------------------------------------------------------------*/
void EwaldCG::reallocate_efield(){
  maxatom = atom->nmax;
  memory->destroy(ek);
  memory->create(ek,maxatom+1,maxatom+1,"kspace:ek");
  //printf("Ek created");
  return;
}
/* ---------------------------------------------------------------------- */

void EwaldCG::deallocate()                                // free memory
{
...
void EwaldCG::init_self_peratom()
{
...
void EwaldCG::compute(int eflag, int vflag)
{
...
void EwaldCG::compute_energy_peratom()
{
...
void EwaldCG::compute_virial_dipole()
{
  if (!function[3]) return;
...
void EwaldCG::compute_virial_peratom()
{
...
double EwaldCG::NewtonSolve(double x, double Rc,
...
double EwaldCG::f(double x, double Rc, bigint natoms, double vol, double b2)
{
  double a = Rc*x;
  double f = 0.0;
...
double EwaldCG::derivf(double x, double Rc,
                         bigint natoms, double vol, double b2)
{
